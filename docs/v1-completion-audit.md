...
| Packaged `/resume` acceptance is included in the V1 aggregate gate | `npm run verify:v1-core` includes `verify:tui-resume-selector-built`, `verify:tui-resume-legacy-candidate`, `verify:tui-resume-current-candidate`, `verify:tui-resume-interrupted-blocked`, `verify:tui-resume-empty-selector`, and `verify:tui-resume-cross-target` alongside the source selector and runnable-preflight resume checks | Covered |
