...
| `/resume` in the TUI opens a selectable recovery page instead of blindly continuing the displayed run | `npm run verify:tui-resume-selector-pty` types `/resume` in a real PTY, shows runnable/blocked candidates, selects `.thinkless2`, and verifies new supervisor events in that selected session | Covered |
