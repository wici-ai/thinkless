...
This file keeps the detailed operational notes, verifier matrix, bootstrap behavior, release policy, and legacy compatibility notes out of the root README. The root README is intentionally short for first-time users and contributors.
...
The name is literal. Thinkless is not anti-thinking; it is against making the human do all of the thinking at the wrong time. Modern AI coding tools often reward prompt polishing, constant supervision, and rewriting intent until it sounds like an execution plan. Thinkless takes the opposite position: the user should express intent naturally, the planner should turn that intent into durable reasoning, and the executor should carry out the work while the user steers only when judgment is needed.

## Philosophy

Software work has different modes that should not be collapsed into one prompt box:

- Intent capture should feel like conversation. Say what you want, including rough edges, constraints, and corrections.
- Planning should be explicit, reviewable, and written down in `GOAL.md`, `ASSUMPTIONS.md`, and `PLAN.md`.
- Execution should be handled by an agent that can inspect the repository, run commands, read logs, recover from failures, and continue.
- Steering should be additive. A later message changes the live goal and plan instead of asking the user to rewrite the original prompt.

Thinkless asks people to think less while coding because most of the extra thinking is not product judgment; it is control labor. Users should not need to decide which command to run after every error, remember every branch the agent tried, or write meta-instructions like "search the docs", "debug if it fails", or "update the plan and keep going". Those are system responsibilities.

The human still does the important thinking: naming the desired outcome, setting boundaries, choosing tradeoffs, reviewing evidence, and stopping or redirecting the run. Thinkless moves machine-shaped thinking into machine-readable artifacts and keeps human-shaped thinking in normal language.

Submitted chat is treated like speech. Submitted turns should be append-only: in a meeting, you do not edit the sentence you already said; you clarify it with the next sentence, and the history remains useful because everyone can see how the intent evolved. Thinkless follows that discipline for coding work: preserve the transcript, preserve the raw requirement, and steer forward. This makes the run auditable and lets the planner iteratively enhance the plan from real evidence instead of pretending the first prompt was a perfect spec.

The plan is not a ceremony or a one-time contract. It is a living working document. The first `PLAN.md` should be good enough to start, and then the planner and executor should improve it as the repository, logs, remote machines, benchmarks, and user follow-ups reveal more. A failed command is not the end of the goal; it is new evidence that should tighten the next step.
...
```text
You: Build a local dashboard for reviewing support tickets and make sure it runs.
Thinkless: captures the goal, asks only essential questions, writes GOAL.md / ASSUMPTIONS.md / PLAN.md, then lets Codex build, test, and report evidence.
You: Also make the mobile layout dense enough for triage.
Thinkless: updates the existing goal and plan, then steers the active execution instead of starting over.
```

Remote performance work:

```text
You: Try the model on this SSH host and see if it can reach 700 token/s.
Thinkless: does not require you to add "look up docs", "benchmark first", or "try another path if it fails". The planner records assumptions and fallback strategy; Codex performs the remote discovery, setup, measurement, and repair loop.
```

Ordinary repository work:

```text
You: What is causing this test to fail?
Thinkless: Chat can inspect the code and answer directly when the work is bounded. If the fix becomes a long-running implementation or validation loop, Chat hands it to planner/executor.
```
...
Thinkless does not parse natural-language requirements into task-specific schemas. If the user says "700 token/s" or asks to build an app, that raw requirement goes into `GOAL.md` once Chat decides it needs planner/executor. Bounded SSH/code-reading, ordinary code changes, validation, commits, pushes, and guarded release commands can stay in Chat when the user asks for them; a longer remote deployment, benchmark, optimization, or iterative debug loop becomes planner/executor work.

Chat agents run with normal native CLI permission to complete bounded direct work themselves: Claude Chat is not forced into plan-only mode, and Codex Chat bypasses approvals and sandboxing like the executor path. Chat should use repository guardrails for risky operations instead of inventing Chat-only bans.
...
`npm run verify:direct-executor-epoch` covers adaptive short executor epochs: fake Codex cumulative token usage crosses the context pressure threshold, WiCi starts fresh executor threads from durable handoff state, and ledger cost records per-turn token deltas instead of cumulative totals.
...
If you are already in the TUI, type `/resume` in the bottom Chat input to open an in-TUI selector. The selector lists the current target, numbered `.thinklessN` sessions, compatible legacy `.wici` runs, and recent Thinkless workspaces. Use up/down to choose a run, Enter to launch a runnable candidate, or Escape to cancel. Each row shows whether the candidate is runnable or blocked, the supervisor state, the selected session directory when relevant, and a short reason. Full supervisor candidates are runnable when Thinkless can restore the selected context or intentionally rerun an interrupted planner/executor path from durable `GOAL.md`, `PLAN.md`, checkpoint, ledger, and session state. Chat-only candidates are also runnable: selecting one restores the Chat transcript/session/runtime context without launching the supervisor. Blocked states, such as a pending planner clarification without a stored planner session, are visible but cannot be launched as resume.

`npm run verify:resume-selector` covers catalog discovery and preflight labels. `npm run verify:tui-resume-selector-pty` covers the source PTY `/resume` selector flow. `npm run verify:tui-resume-selector-built` drives the packaged CLI entrypoint, proves up/down selection, Enter launch, Escape cancellation, and chat-only selection without `RESUME_CONTEXT_VALIDATED`, `SUPERVISOR_START`, or `EXECUTOR_RESUME_FALLBACK` side effects. `npm run verify:tui-resume-legacy-candidate` covers built-CLI legacy `.wici` selection and proves runnable legacy candidates preserve full context while legacy chat-only candidates restore Chat without mutating decoys. `npm run verify:tui-resume-current-candidate` covers the current `.thinkless` candidate, proving `/resume` opens the selector without implicit launch and only emits `RESUME_CONTEXT_VALIDATED` plus `SUPERVISOR_START` after the current row is explicitly selected. `npm run verify:tui-resume-interrupted-blocked` covers packaged selector refusal for checkpointed but unrecoverable planner/executor interruptions, including a pending planner clarification without a persisted planner session and an executor interruption without replayable state. `npm run verify:tui-resume-interrupted-runnable` covers packaged selector launch for interrupted-but-runnable planner and executor paths: planner rerun candidates emit `RESUME_CONTEXT_VALIDATED` with `planner_rerun` plus `SUPERVISOR_START` and no `RESUME_CONTEXT_BLOCKED`, while executor replay candidates emit `RESUME_CONTEXT_VALIDATED`, `EXECUTOR_RESUME_FALLBACK`, and `SUPERVISOR_START`; both preserve full context and leave decoys unchanged. `npm run verify:tui-resume-command-isolation` covers `/resume` as a bottom Chat command, proving the selector opens, Escape cancels cleanly, no Chat transcript/inbox/GOAL.md/PLAN.md/checkpoint/ledger/current-session event or Chat-agent steering mutation occurs before explicit selection, and later runnable selection mutates only the selected candidate while preserving full context. `npm run verify:tui-resume-planner-context` covers selected-run planner self-interrogation context, proving `ASSUMPTIONS.md` and planner `context.md` come from the selected candidate, current-target and decoy assumptions/context/events remain unmutated, and selected-session `RESUME_CONTEXT_VALIDATED` plus `SUPERVISOR_START` preserve planner/executor/app-thread metadata. `npm run verify:tui-resume-empty-selector` covers the packaged selector empty-catalog path, proving `/resume` opens an explicit empty recovery page, Enter/up/down do not launch, and Escape cancels cleanly. `npm run verify:tui-resume-many-candidates` covers a catalog with more than eight discovered candidates and proves Enter launches only the selected runnable session while decoys remain unchanged. `npm run verify:tui-resume-stale-candidate` covers launch-time revalidation when a previously runnable candidate loses durable state after the selector opens, proving it blocks with `RESUME_CONTEXT_BLOCKED` instead of launching read-only history. `npm run verify:tui-resume-stale-agent-state` covers launch-time revalidation when a previously runnable planner/executor candidate loses persisted planner stdout or executor `codex-run.jsonl` transcript state before Enter, proving the TUI blocks with a concrete planner/executor reason, emits no `RESUME_CONTEXT_VALIDATED`, `SUPERVISOR_START`, or `EXECUTOR_RESUME_FALLBACK`, and leaves runnable decoys unchanged. `npm run verify:tui-resume-blocked-then-runnable` covers same-page selector recovery after a blocked or stale selection: the blocked candidate emits only `RESUME_CONTEXT_BLOCKED`, remains non-launched, then a runnable decoy selected from the same `/resume` page emits `RESUME_CONTEXT_VALIDATED` and `SUPERVISOR_START` with full Chat/runtime/planner/executor/app-thread context. `npm run verify:tui-resume-cross-target` covers selecting a historical workspace run while appending `RESUME_CONTEXT_VALIDATED` and `SUPERVISOR_START` only to the selected target/session and preserving Chat, planner, executor, checkpoint, GOAL, PLAN, ledger, and app-thread metadata. `npm run verify:resume-rerunnable` covers blocked planner and executor rerun/fallback events (`RESUME_CONTEXT_VALIDATED`, `RESUME_CONTEXT_BLOCKED`, and `EXECUTOR_RESUME_FALLBACK`). The core V1 aggregate, `npm run verify:v1-core`, includes the built selector, legacy `.wici`, current `.thinkless`, interrupted blocked planner/executor, interrupted runnable planner/executor, command-isolated Chat `/resume`, selected planner self-interrogation context, empty-catalog selector, many-candidate catalog launch, stale-candidate launch revalidation, stale planner/executor durable-state revalidation, blocked-then-runnable same-page recovery, and cross-target resume checks so packaged `/resume` acceptance stays in the release-facing gate.
