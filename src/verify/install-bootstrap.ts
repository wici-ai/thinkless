...
async function main(): Promise<void> {
  ...
  assert(readme.includes('Apple Command Line Tools') && readme.includes('~/.zprofile') && readme.includes('~/.zshrc') && readme.includes('`thinkless`, `codex`, `claude`, and `gh`') && readme.includes('clean zsh login and interactive shells'), 'README must document first-run CLT waiting and full zsh command exposure');
  assert(readme.includes('auth onboarding flow') && readme.includes('THINKLESS_AUTH_ONBOARDING=0') && readme.includes('/dev/tty'), 'README must document installer auth onboarding and opt-out');
