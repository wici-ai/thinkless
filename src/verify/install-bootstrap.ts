...
async function main(): Promise<void> {
  ...
  assert(bootstrap.includes('print_path_activation_note') && bootstrap.includes('open a new terminal') && bootstrap.includes('export PATH=\\"$bin:\\$HOME/.local/bin:/opt/homebrew/bin:/usr/local/bin:\\$PATH\\"'), 'zero-npm macOS bootstrap must tell users how to activate PATH in the current terminal');
  ...
  assert(bootstrap.includes('THINKLESS_AUTH_ONBOARDING') && bootstrap.includes('/dev/tty') && bootstrap.includes('gh auth login') && bootstrap.includes('run_tty "claude"'), 'zero-npm macOS bootstrap must offer terminal-backed auth onboarding for Codex, Claude, and GitHub CLI');
  assert(bootstrap.includes('THINKLESS_AUTH_PENDING=1') && bootstrap.includes('auth is pending') && bootstrap.includes('OPENAI_API_KEY') && bootstrap.includes('ANTHROPIC_API_KEY'), 'zero-npm macOS bootstrap must distinguish installed commands from pending auth');
  ...
  assert(publicInstaller.includes('print_path_activation_note') && publicInstaller.includes('open a new terminal') && publicInstaller.includes('export PATH=\\"$bin:\\$HOME/.local/bin:/opt/homebrew/bin:/usr/local/bin:\\$PATH\\"'), 'public installer must tell users how to activate PATH in the current terminal');
  ...
  assert(publicInstaller.includes('THINKLESS_AUTH_ONBOARDING') && publicInstaller.includes('/dev/tty') && publicInstaller.includes('gh auth login') && publicInstaller.includes('run_tty "claude"'), 'public installer must offer terminal-backed auth onboarding for Codex, Claude, and GitHub CLI');
  assert(publicInstaller.includes('THINKLESS_AUTH_PENDING=1') && publicInstaller.includes('auth is pending') && publicInstaller.includes('OPENAI_API_KEY') && publicInstaller.includes('ANTHROPIC_API_KEY'), 'public installer must distinguish installed commands from pending auth');
  ...
  assert(readme.includes('Apple Command Line Tools') && readme.includes('~/.zprofile') && readme.includes('~/.zshrc') && readme.includes('`thinkless`, `codex`, `claude`, and `gh`') && readme.includes('clean zsh login and interactive shells') && readme.includes('open a new terminal'), 'README must document first-run CLT waiting and full zsh command exposure');
  assert(readme.includes('auth onboarding flow') && readme.includes('THINKLESS_AUTH_ONBOARDING=0') && readme.includes('/dev/tty') && readme.includes('auth is pending'), 'README must document installer auth onboarding, opt-out, and pending-auth state');
