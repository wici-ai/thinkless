...
async function main(): Promise<void> {
  ...
  assert(bootstrap.includes('env -i HOME="$HOME"') && bootstrap.includes('/bin/zsh -lc') && bootstrap.includes('/bin/zsh -ic') && bootstrap.includes('for cmd in node npm git thinkless codex claude gh') && bootstrap.includes('THINKLESS_SELF_UPDATE=0') && bootstrap.includes('git --version >/dev/null') && bootstrap.includes('codex --version >/dev/null') && bootstrap.includes('claude --version >/dev/null'), 'zero-npm macOS bootstrap must verify Node, npm, Git, Thinkless, Codex, Claude, and GitHub CLI from clean zsh login and interactive shells');
  ...
  assert(publicInstaller.includes('install_linux_packages') && publicInstaller.includes('apt-get install -y') && publicInstaller.includes('dnf install -y') && publicInstaller.includes('pacman -Sy --needed --noconfirm') && publicInstaller.includes('zypper --non-interactive install') && publicInstaller.includes('apk add --no-cache'), 'public installer must install missing Linux curl, Git, Node.js/npm, and GitHub CLI through common package managers');
  assert(publicInstaller.includes('node:*) package=nodejs') && publicInstaller.includes('npm:*) package=npm') && publicInstaller.includes('gh:pacman|gh:apk) package=github-cli'), 'public installer must map Linux dependency package names for Node.js/npm and GitHub CLI');
  ...
  assert(publicInstaller.includes('env -i HOME="$HOME"') && publicInstaller.includes('/bin/zsh -lc') && publicInstaller.includes('/bin/zsh -ic') && publicInstaller.includes('for cmd in node npm git thinkless codex claude gh') && publicInstaller.includes('THINKLESS_SELF_UPDATE=0') && publicInstaller.includes('git --version >/dev/null') && publicInstaller.includes('codex --version >/dev/null') && publicInstaller.includes('claude --version >/dev/null'), 'public installer must verify Node, npm, Git, Thinkless, Codex, Claude, and GitHub CLI from clean zsh login and interactive shells');
  ...
  assert(windowsInstaller.includes("Ensure-CommandViaWinget 'node' 'OpenJS.NodeJS.LTS'") && windowsInstaller.includes("Ensure-CommandViaWinget 'npm' 'OpenJS.NodeJS.LTS'") && windowsInstaller.includes('Git.Git') && windowsInstaller.includes('GitHub.cli') && windowsInstaller.includes("Invoke-Checked 'winget'") && windowsInstaller.includes("'install'"), 'Windows installer must use winget for missing Node.js/npm, Git, and GitHub CLI');
  ...
  assert(readme.includes('Apple Command Line Tools') && readme.includes('~/.zprofile') && readme.includes('~/.zshrc') && readme.includes('`node`, `npm`, `git`, `thinkless`, `codex`, `claude`, and `gh`') && readme.includes('clean zsh login and interactive shells') && readme.includes('export PATH=... && thinkless'), 'README must document first-run CLT waiting and full zsh command exposure');
  assert(readme.includes('On Linux, `install.sh` uses the available system package manager') && readme.includes('THINKLESS_INSTALL_DEPS=0'), 'README must document Linux dependency installation and opt-out');
