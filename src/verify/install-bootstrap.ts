...
async function main(): Promise<void> {
  ...
  assert(publicReleaseWorkflow.includes('default: wici-ai/thinkless'), 'public release workflow must publish to the public thinkless repo by default');
  ...
  assert(!publicReleaseWorkflow.includes(oldReleaseRepoName), 'public release workflow must not reference the old public release repo name');
  ...
  assert(readme.includes('git clone git@github.com:wici-ai/thinkless-dev.git'), 'README must document the private thinkless-dev source repo');
  assert(!readme.includes(oldReleaseRepoName), 'README must not reference the old public release repo name');
