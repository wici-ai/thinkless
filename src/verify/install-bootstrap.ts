...
async function main(): Promise<void> {
  ...
  assert(publicInstaller.includes('THINKLESS_AUTH_ONBOARDING') && publicInstaller.includes('/dev/tty') && publicInstaller.includes('gh auth login') && publicInstaller.includes('run_tty "$codex_cmd"') && publicInstaller.includes('run_tty "claude"') && authPrompts.every((prompt) => publicInstaller.includes(prompt)), 'public installer must offer terminal-backed auth onboarding for Codex, Claude, and GitHub CLI');
