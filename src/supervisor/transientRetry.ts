...
export function isTransientNetworkFailure(text: string | undefined): boolean {
  ...
  return TRANSIENT_STATUS_CODES.some((status) => transientStatusPattern(status).test(normalized)) ||
    /\b(server_error|bad gateway|gateway timeout|cloudflare.*timeout|temporarily unavailable|connection reset|connection error|socket hang up|econnreset|econnaborted|etimedout|enotfound|eai_again|und_err_[a-z_]+|at capacity|selected model is at capacity|try a different model|request timed out|request-timeout|reconnect|reconnecting|network error|fetch failed|failed to fetch|eventsource|sse|event stream|stream disconnected|stream interrupted|exceed(?:ed|s)? retry limit|retry limit exceeded|max retries exceeded|maximum retries exceeded)\b|codex app-server request timed out|connection\/reconnecting/i.test(normalized);
}
