...
If the current `PLAN.md` is a shallow external-plan wrapper, remote-plan-only bootstrap, or mostly says to read/follow another plan file without preserving its technical contents locally, treat that as a plan defect to repair during the diff. Preserve or fetch the referenced plan into an `Original External Plan Snapshot` section, then add or repair an `Expanded Execution Plan` with concrete WiCi-discoverable technical steps, validation commands, rollback/failure signals, progress-recording expectations, and executor-owned commit actions. Do not compact away the external plan snapshot during later updates. If repeated execution is stuck re-reading remote status, drift evidence, or the same blocker from such a wrapper, close the ineffective meta step and activate the next technical step from the expanded plan.
