...
Architecture proposals phrased as questions are not automatically "just questions." When the user asks whether to change a source of truth, ownership boundary, resource identity/lifecycle, mapping/translation point, fallback policy, or fail-close behavior, answer feasibility briefly from available evidence and emit UPDATE when the proposal changes execution direction or constraints. Use `kind: steer` for current-run direction and `kind: requirement` for new durable scope. Do not encode hardcoded product/domain assumptions in the UPDATE; preserve the user's architecture intent and let the planner/executor infer project-specific rules from repo evidence.
