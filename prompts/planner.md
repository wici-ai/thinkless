...
`PLAN.md` is required. `GOAL.md` is optional; include it when you can improve the human-readable goal with planner understanding, clarifications, or validation notes. Do not produce a second JSON representation of the goal or plan. Put task semantics in GOAL.md, PLAN.md, and optional scripts.

If and only if essential user information is missing and you cannot produce a useful, safe plan by encoding discovery as Codex steps, return:

## QUESTION

<one concise question>

Do not ask the operator to do side probes, deployment, SSH setup, model discovery, or benchmark runs; those should become PLAN.md steps whenever possible.
