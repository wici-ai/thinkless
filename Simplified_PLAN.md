...
- TUI：底部独立 Chat 输入框 + 上方可左右切换的 Chat 历史 / Goal/Plan / Execution 工作区。
- 三个工作区有可见的 TUI 选择器，允许选择各自的 agent(command) 和 effort；agent 只能是 Claude 或 Codex，model 由 agent 固定派生：Claude 用 claude-opus-4-8，Codex 用 gpt-5.5。
...
- tag 前必须跑完整自动 V1 core preflight，并通过 guarded tag 命令创建本地 annotated tag。

核心原则：

- 用户从 Chat 输入自然语言；第一句不必然是目标，也可以是让 Chat agent 先阅读代码库、解释状态或讨论方案。
- Chat agent 在用户要求 plan，或自己判断需求信息足够时，才触发 planner。
...
1. 启动本地 TUI。
2. 初始状态下上方 Chat 历史 / Goal/Plan / Execution 工作区为空，底部 Chat 是唯一输入入口。
3. 用户可以先让 Chat agent 阅读代码库或讨论方案；如果已经有明确需求，也可以直接输入需求，例如：

   ```text
   听说diffusionGemma很快，在ssh -p 23276 root@116.127.115.18 -L 8080:localhost:8080试试，要求达到700 token/s以上
   ```

4. Chat agent 判断信息足够或用户要求 plan 时，发出 update；WiCi 才把该需求写入面向人的 `GOAL.md`，不把 `700 token/s`、SSH 命令、应用类型等解析成 WiCi 自己的专用 JSON 语义。
5. WiCi 启动 Claude Code plan mode；信息不足时，Claude 的澄清问题回到 Chat，用户直接在 Chat 回复。
...
- 不能在完整自动 V1 core preflight 通过前 tag 或 push。
...
- 本地 TUI：底部 Chat 输入框；上方 Chat 历史、Goal/Plan、Execution 在同一个工作区内左右切换。
...
- tag 前必须跑完整自动 V1 core preflight。

## V1 发布验证

每次 tag 前必须运行发布 preflight：

```bash
npm run release:preflight
```

该命令运行自动 V1 core gate。创建 release tag 必须使用 guarded command：

```bash
npm run release:tag -- 0.1.0
```

`release:tag` 先运行 preflight，再创建 annotated local tag；它不会 push。发布证据来自 `verify:v1-core` 覆盖的 Chat-first intake、planner、executor、resume、hot reload、rollback、secret scan 和 release tag 行为检查。
